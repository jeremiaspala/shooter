...
if ($id <= 0) {
    $_SESSION['toast'] = ['type' => 'error', 'message' => 'ID de usuario inválido'];
...
if (!$user) {
    $_SESSION['toast'] = ['type' => 'error', 'message' => 'Usuario no encontrado'];
    header('Location: /shooter/users');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $_SESSION['toast'] = ['type' => 'success', 'message' => 'Usuario actualizado correctamente'];
        header('Location: /shooter/users');
        exit;
    }
}
...
<div class="card">
    ...
    <form method="POST">
        <div class="form-group">
            ...
            <input type="text" id="username" name="username" class="form-control" value="<?php echo htmlspecialchars($_POST['username'] ?? $user['username']); ?>" required>
        </div>
        
        <div class="form-group">
            <label for="email">Email *</label>
            <input type="email" id="email" name="email" class="form-control" value="<?php echo htmlspecialchars($_POST['email'] ?? $user['email']); ?>" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select id="role" name="role" class="form-control">
                <option value="visor" <?php echo ($_POST['role'] ?? $user['role']) === 'visor' ? 'selected' : ''; ?>>Visor (solo lectura)</option>
                <option value="admin" <?php echo ($_POST['role'] ?? $user['role']) === 'admin' ? 'selected' : ''; ?>>Admin (acceso completo)</option>
            </select>
        </div>
